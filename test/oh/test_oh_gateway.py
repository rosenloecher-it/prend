...
class TestOhGateway(unittest.TestCase):
    ...
    def test_cache_states_if_needed(self):

        class TestGateway2(OhGateway):
            def __init__(self):
                super().__init__(TestDispatcher(), TestRest())
                self.cache_states_was_executed = False

            def cache_states(self):
                # is done also in super func !!!
                self._cache_states_notified_reload = None

                self.cache_states_was_executed = True

        gateway = TestGateway2()

        gateway.cache_states_was_executed = False
        gateway._cache_states_notified_reload = None
        gateway._cache_states_last_fetch = None
        out = gateway.cache_states_if_needed()
        self.assertTrue(out)
        self.assertEqual(out, gateway.cache_states_was_executed)

        # force not yet
        gateway.cache_states_was_executed = False
        gateway._cache_states_notified_reload = None
        diff = gateway.TIME_FORCE_RELOAD_SEC/2
        gateway._cache_states_last_fetch = datetime.datetime.now() - datetime.timedelta(seconds=diff)
        out = gateway.cache_states_if_needed()
        ...
        gateway.cache_states_was_executed = False
        gateway._cache_states_notified_reload = None
        diff = gateway.TIME_FORCE_RELOAD_SEC * 1.5
        gateway._cache_states_last_fetch = datetime.datetime.now() - datetime.timedelta(seconds=diff)
        out = gateway.cache_states_if_needed()
        self.assertTrue(out)
        self.assertEqual(out, gateway.cache_states_was_executed)

        # wait for another notification - not yet
        gateway.cache_states_was_executed = False
        diff = gateway.TIME_WAIT_ANOTHER_RELOAD_SEC / 2
        gateway._cache_states_notified_reload = datetime.datetime.now() - datetime.timedelta(seconds=diff)
        gateway._cache_states_last_fetch = datetime.datetime.now()
        out = gateway.cache_states_if_needed()
        self.assertFalse(out)
        self.assertEqual(out, gateway.cache_states_was_executed)

        # wait for another notification - now
        gateway.cache_states_was_executed = False
        gateway._cache_states_notified_reload = datetime.datetime.now()
        diff = gateway.TIME_WAIT_ANOTHER_RELOAD_SEC * 1.5
        gateway._cache_states_notified_reload = datetime.datetime.now() - datetime.timedelta(seconds=diff)
        diff = gateway.TIME_FORCE_RELOAD_SEC / 2
        gateway._cache_states_last_fetch = datetime.datetime.now() - datetime.timedelta(seconds=diff)
        out = gateway.cache_states_if_needed()
        ...
        gateway.cache_states_was_executed = False
        gateway._cache_states_notified_reload = datetime.datetime.now()
        diff = gateway.TIME_WAIT_LAST_RELOAD_SEC * 2
        gateway._cache_states_last_fetch = gateway._cache_states_notified_reload - datetime.timedelta(seconds=diff)
        out = gateway.cache_states_if_needed()
        self.assertFalse(out)
        self.assertEqual(out, gateway.cache_states_was_executed)
